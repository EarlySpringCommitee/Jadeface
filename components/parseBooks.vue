	...
	<v-container grid-list-md v-if="books.length>0">
			...
			<v-flex v-for="item in books" :key="item.uuid" xs6 md3>
				<v-item>
					<v-card>
						...
						<v-card-text>{{item.desc.length>150?(item.desc.substr(0,150)+'...'):item.desc}}</v-card-text>
						<v-card-actions>
		...
		<v-data-table :headers="headers" :items="books" sort-by="name" multi-sort v-if="view==='list'">
			<template v-slot:item.desc="{ item }">
				<p>{{item.desc.length>60?(item.desc.substr(0,60)+'...'):item.desc}}</p>
			</template>
					...
					<v-container grid-list-sm>
						<v-layout wrap>
							...
							<v-flex xs12 sm12 md12>
								<v-text-field outlined v-model="editedBook.cover_id" readonly label="cover_id" />
							</v-flex>
							<v-layout justify-space-between row>
								<v-text-field outlined v-model="editedBook.filepath" readonly label="filepath" />
								<v-btn color="blue darken-1" icon @click="download">
